...
chi.sq <- function(df, var1, var2, correct = FALSE, post = FALSE, plot = FALSE, cramer = FALSE){
  ...
  if(post == TRUE# | plot == TRUE
    ...
    post_mat_rownames <- paste0(v1,": ",dimnames(stdres)[[1]]) #var1
    post_mat_colnames <- paste0(v2,": ",dimnames(stdres)[[2]]) #var2
  ...
  if(plot==TRUE){
    ...
    post_mat_rownames <- paste0(deparse(substitute(var1)),": ",dimnames(stdres)[[1]]) #var1
    post_mat_colnames <- paste0(deparse(substitute(var2)),": ",dimnames(stdres)[[2]]) #var2
    #print(post_mat_colnames)
